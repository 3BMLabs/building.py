...
def keydown(e):
    ...
    if e.char == "a":
        ...
        child_offsets: list[tuple[int, float] | None] = sf.fit_lengths_1d(
            parent_lengths, child_lengths, padding, allowed_error
        )
        ...
        for parent_length in parent_lengths:
            canvas.create_rectangle(
                0, yoff, parent_length, yoff + vert_size, fill="red"
            )
            yoff += vert_size
        ...
        for child_off, child_length in zip(child_offsets, child_lengths):
            ...
            if child_off != None:
                ...
                canvas.create_rectangle(
                    child_off[1],
                    child_y_off,
                    child_off[1] + child_length,
                    child_y_off + vert_size,
                    fill="blue",
                )
            else:
                canvas.create_rectangle(
                    0, yoff, child_length, yoff + vert_size, fill="orange"
                )
                yoff += vert_size
        ...
        canvas.create_text(
            ...
            text=f"minimum cost: {mincost}\tcost (including cutting cost): {cost}\tloss: {((cost / mincost) - 1) * 100}%",
            anchor="nw",
        )
    elif e.char == "s":
        # 2d
        parent_sizes = [
            Vector(random.randrange(10, 200, 10), random.randrange(10, 200, 5))
            for i in range(parent_count)
        ]
        child_sizes = [
            Vector(random.randrange(10, 50, 10), random.randrange(10, 50, 5))
        ...
        parent_offsets = sf.fit_boxes_2d(
            [Vector(geo_width, geo_height)], parent_sizes, 50, 100
        ).fitted_boxes
        ...
        for parent_offset, parent_size in zip(parent_offsets, parent_sizes):
            if parent_offset != None:
                canvas.create_rectangle(
                    parent_offset[1].x,
                    parent_offset[1].y,
                    parent_offset[1].x + parent_size.x,
                    parent_offset[1].y + parent_size.y,
                    fill=("red" if grandparent_array_off < parent_count else "orange"),
                )
                ...
                if parent_offsets[child_off[0]] != None:
                    ...
                    if show_padding:
                        canvas.create_rectangle(
                            padded_rect.x,
                            padded_rect.y,
                            padded_rect.x + padded_rect.size.x,
                            padded_rect.y + padded_rect.size.y,
                            fill="green",
                        )
                    canvas.create_rectangle(
                        child_rect.x,
                        child_rect.y,
                        child_rect.x + child_rect.size.x,
                        child_rect.y + child_rect.size.y,
                        fill="blue",
                    )
        if show_leftovers:
        ...
        available_parent_invoicing_widths = [
            2450,
            2500,
            2730,
            2950,
        ...
        parent_sizes = [
            Vector(
                random.uniform(min_length, max_length),
                random.choice(available_parent_invoicing_widths),
            )
            for i in range(parent_count)
        ]
        ...
        parent_offsets = sf.fit_boxes_2d(
            [Vector(canvas_width * scale_factor, canvas_height * scale_factor)],
            parent_sizes,
            50,
            100,
        ).fitted_boxes

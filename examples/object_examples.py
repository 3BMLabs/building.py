...
# Arc - start
arcPlane = Plane(
    origin=Point.from_coords(200, 30, 13),
    normal=Vector.from_coords(0, 0, 1),
    xdir=Vector.from_coords(1, 0, 0),
    ydir=Vector.from_coords(0, 1, 0),
    units="mm",
)
arcInterval = Interval(start=0, end=1, totalChildrenCount=1)
arcObj = Arc(
    startPoint=Point(x=0, y=20, z=13),
    midPoint=Point(x=500, y=0, z=13),
    endPoint=Point(x=1000, y=20, z=13),
    plane=arcPlane,
    radius=20,
...
# Circle - start
circlePlane = Plane(
    origin=Point(x=350, y=35, z=13, units="mm"),
...
# Ellipse - start
ellipsePlane = Plane(
    origin=Point(x=1, y=0, z=13, units="mm"),
...
ellipseObj = Ellipse(
    firstRadius=40.0, secondRadius=20.0, plane=ellipsePlane, units="mm"
)
...
spiralObj = Spiral(
    ...
    plane=spiralPlane,
    turns=4,
    pitchAxis=Vector(x=0, y=1, z=0),
    spiralType=SpiralType.BiquadraticParabola,
)
...
arcPlane = Plane(
    origin=Point.from_coords(0 + p, 0 + xp, 13),
    normal=Vector.from_coords(1, 0, 0),
    xdir=Vector.from_coords(0, 1, 0),
    ydir=Vector.from_coords(0, 1, 0),
    units="mm",
)
arcInterval = Interval(start=0, end=1, totalChildrenCount=1)
arcObj = Arc(
    startPoint=p1,
    midPoint=Point(x=250 + p, y=40 + xp, z=13),
    endPoint=p2,
    plane=arcPlane,
    radius=20,
    interval=arcInterval,
...
# Polyline - start
coordZ = 13
polylineObj = Polyline.from_points(
    [
        Point(x=175, y=175, z=coordZ, units="mm"),
        Point(x=225, y=175, z=coordZ, units="mm"),
        Point(x=225, y=200, z=coordZ, units="mm"),
        Point(x=175, y=200, z=coordZ, units="mm"),
        Point(x=175, y=175, z=coordZ, units="mm"),
    ]
)
...
meshObj = Mesh(vertices=allVertices, faces=allFaces, units="mm")
# Mesh - end
...
def Platform(height, xyz, btmShape=None, text=None, txyz=None):
    ...
    meshPlatform = Mesh(vertices=allVertices, faces=allFaces, units="mm")
    if text != None:
...
MeshBase1 = Platform(
    2, [0, 0, 10], btmShape="btmShape", text="LINE", txyz=[0 + 20, 0 + 50, 10]
)
objList.append(MeshBase1)

MeshBase2 = Platform(
    2, [150, 0, 10], btmShape="btmShape", text="ARC", txyz=[150 + 18, 0 + 50, 10]
)
...
MeshBase3 = Platform(
    ...
    btmShape="btmShape",
    text="POLYLINE",
    txyz=[150 + 0, 150 + 50, 10],
)
objList.append(MeshBase3)

MeshBase4 = Platform(
    2, [0, 150, 10], btmShape="btmShape", text="POINT", txyz=[0 + 15, 150 + 50, 10]
)
objList.append(MeshBase4)

MeshBase5 = Platform(
    2, [300, 0, 10], btmShape="btmShape", text="CIRCLE", txyz=[300 + 10, 0 + 50, 10]
)
objList.append(MeshBase5)

MeshBase6 = Platform(
    2, [300, 150, 10], btmShape="btmShape", text="POLYCURVE", txyz=[290, 150 + 50, 10]
)
objList.append(MeshBase6)

MeshBase7 = Platform(
    2, [450, 0, 10], btmShape="btmShape", text="MESH2", txyz=[450 + 10, 0 + 50, 10]
)
objList.append(MeshBase7)

MeshBase8 = Platform(
    2, [450, 150, 10], btmShape="btmShape", text="MESH1", txyz=[450 + 10, 150 + 50, 10]
)
...
send_to_speckle(
    host="https://3bm.exchange", stream_id="fa4e56aed4", objects=objList, msg="Objects"
)

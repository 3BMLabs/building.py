...
class Level:
    ...
    @classmethod
    def byPoint(self, point=Point, name=str):
        if point.type == "Point":
            Lvl = Level()
            XY_plane = [Vector3(x=1, y=0, z=0), Vector3(x=0, y=1, z=0)]
            Lvl.plane = Plane.byTwoVectorsOrigin(XY_plane[0], XY_plane[1], point)
            Lvl.polycurve = RectXY(Point.toVector(point), 1000, 1000)

...
class Arc(Curve):
    ...
    def by_start_mid_end(start: Point, mid: Point, end: Point) -> 'Arc|Line':
        ...
        # triangle normal
        perpendicular_vector = Vector.cross_product(start_to_mid, start_to_end)
        wls = perpendicular_vector.length_squared
        if (wls<10e-14):
            return None # area of the triangle is too small (you may additionally check the points for colinearity if you are paranoid)

        # helpers
        iwsl2 = 1.0 / (2.0*wls)
        tt = start_to_mid*start_to_mid
        uu = start_to_end*start_to_end

        # result circle
        origin = start + (start_to_end*tt*(start_to_end*mid_to_end) - start_to_mid*uu*(start_to_mid*mid_to_end)) * iwsl2
        #radius = math.sqrt(tt * uu * (mid_to_end*mid_to_end) * iwsl2*0.5)
        normalized_z_axis = perpendicular_vector / math.sqrt(wls)
        x_axis = start - origin 
        normalized_x_axis = x_axis.normalized
        radius = normalized_x_axis.length

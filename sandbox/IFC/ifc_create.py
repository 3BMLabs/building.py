...
# write me a function that calculates the angle over the axis (x or y or z) based on 2 points.
...
# Use an arbitrary vector for the initial cross product; choosing Z-axis unit vector
arbitrary_vector = np.array([0, 0, 1])

# Calculate an orthogonal vector to X, which we'll adjust to be the Z vector
vector_y_initial = np.cross(vector_x, arbitrary_vector)

# If the initial Y is parallel to X (unlikely with the chosen arbitrary vector but to ensure generality), adjust the arbitrary vector
if not np.any(vector_y_initial):
    arbitrary_vector = np.array([0, 1, 0])
    vector_y_initial = np.cross(vector_x, arbitrary_vector)

# Calculate the Z vector orthogonal to both X and the initial Y vector
vector_z = np.cross(vector_x, vector_y_initial)

# Normalize the Y and Z vectors to get unit vectors
...
matrix = ifcopenshell.util.placement.a2p((0,0,0), (0,0,0), (vector_z[0],vector_z[1],vector_z[2])) @ matrix
# matrix = ifcopenshell.util.placement.a2p((0,0,0), (vector_y[0],vector_y[1],vector_y[2]), (vector_z[0],vector_z[1],vector_z[2])) @ matrix
print(matrix)

matrix[:,3][0:3] = (p2.x, p2.y, p2.z)

...
pat_prefix = ";%UNITS=MM" ";"
revit_model_pattern = ";%TYPE=MODEL"
...
class PATRow:
    ...
    def create_pat_string(self):
        patstr = (
            ...
            + str(self.y_orig)
            + ",  "
            + str(self.shift_pattern)
            + ",  "
            + str(self.offset_spacing)
        )
        if self.dash == 0:
            addstr = ""
        else:
...
class PAT:
    ...
    @classmethod
    def TilePattern(cls, name: str, width: float, height: float, patterntype: str):
        self = cls(name)
    ...
    def BlockPattern(
        self, name: str, grosswidthheight: float, numbersublines: int, patterntype: str
    ):
    ...
    def ChevronPattern(
        self, name: str, grosswidth: float, widthtile: float, patterntype: str
    ):
    ...
    def HerringbonePattern(
        self,
        name: str,
        lengthtile: float,
        numberOfTilesInLength: float,
        patterntype: str,
    ):
    ...
    def StretcherBondPattern(
        self, name: str, width: float, height: float, shift: float, patterntype: str
    ):
...
def CreatePatFile(patternobjects: list, filepath: str):
    ...
    patternstrings.insert(0, pat_prefix)
...
def PatRowGeom(patrow: PATRow, width: float, height: float, dx, dy):
    ...
    for i in range(nlines):
        CSNewLn = CoordinateSystem.rotate(math.radians(patrow.angle), Vector.up)
        l1 = Line(Point(0, 0, 0), Point(width, 0, 0))  # baseline
        l2 = CSNewLn * l1  # rotation
        ...
        #    l3 = Line.translate_2(l2,v2) # shift of line for pattern
        v3 = Vector.cross_product(v1, Vector.z_axis).normalized  # Eenheidsvector haaks op lijn
        if patrow.angle == 0:
            v4 = Vector.scale(
                v3, n * patrow.offset_spacing
            )  # Verplaatsingsvector voor spacing, inverse in geval lijn = 0 graden
            v4 = Vector.reverse(v4)
        else:
            v4 = Vector.scale(
                v3, n * patrow.offset_spacing
            )  # Verplaatsingsvector voor spacing
